...
import org.safedu.danger.datamng.YPYNetUtils;
import org.safedu.danger.geocode.GeoCodeCalc;
import org.safedu.danger.object.PlaceObject;
...
public class DangerInTownActivity extends DangerActivity implements OnMapReadyCallback, LocationListener,
	...
	private final static String TAG = DangerInTownActivity.class.getName();
		...
		handler.post(new Runnable() {
			...
			public void run() {
				if ( googleApiClient == null ) {
					googleApiClient = new GoogleApiClient.Builder(DangerInTownActivity.this)
					.addConnectionCallbacks(DangerInTownActivity.this)
					.addOnConnectionFailedListener(DangerInTownActivity.this)
		...
		handler.post(new Runnable() {
			...
			public void run() {
				if (buttonStatus == BUTTON_ENABLED) {
					LocationServices.FusedLocationApi.requestLocationUpdates(googleApiClient, REQUEST, DangerInTownActivity.this); // LocationListener
				} else {
					LocationServices.FusedLocationApi.requestLocationUpdates(googleApiClient, REQUEST2, DangerInTownActivity.this); // LocationListener
				}
...
//				ResponsePlaceResult rpr = YPYNetUtils.getListPlacesBaseOnText(DangerDataTest.this, mLatLng.longitude, mLatLng.latitude, txtSearch.getText().toString());
				ResponsePlaceResult rpr = YPYNetUtils.getListPlacesBaseOnText(DangerInTownActivity.this, latLng.longitude, latLng.latitude, query);
